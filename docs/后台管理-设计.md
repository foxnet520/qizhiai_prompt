# 第一部分：产品整体设计

## 目标：

搭建一个“模型聚合访问器 + SaaS 平台”， **AI 聚合对话服务 + 接口入口 + 商用层**。

## 产品思路：

---------

### 📌 **核心架构**

```
用户输入（前端 UI)  
   ↓  
调用后端调度服务
   ↓  
根据策略调用不同模型：
   - OpenAI 官方 API（合法付费）
   - 开源模型（本地部署/远程 API）
   - 自己可接入其他模型
   ↓  
返回输出→前端展示
```

### 📌 付费/权限策略层

- 免费 token 数
- 注册后更高额度
- 会员付费解锁更多模型

### 📌 自主运营特点

- 多模型对比
- 压力测验
- 统一账单/限流系统

------

### 业务定位：

 🟡 “国内可访问的 AI 工具集成平台 + 统一账号管理 + 可用量可控 + 模型对比与能力选择”.

### 目标用户：

- 创作者、内容工作者
- 中小企业（客服/客服助理）
- 个人用户（写作、代码、资料生成）
- AI 工具试用者

### 策略定位：

做一个**“AI 模型聚合与商业付费平台”**
 ➡ 不依赖绕过 VPN 或违规镜像
 ➡ 合法接入 OpenAI API、Anthropic、Google Gemini、Claude 等（按 API 付费）
 ➡ 用户付费使用额度、会员包

### 核心产品线（可直接变现）

#### **A)  AI 对话助手（泛任务版）**

可用于：

- 问答、写作辅助
- 文案生成
- 数据分析问答
- 代码生成
- 学习辅助

**付费模式**：

- 免费额度
- 包天/包月会员
- 次数付费（消耗多少付多少）

#### **B) 多模型对比实验室（专业版）**

用户可选择多个模型：

- 输入同一 prompt
- 横向对比输出

价值卖点：
 “哪家模型更擅长你的任务？”
 适用于：

- 文案选题
- 策划比较
- 创意优化

#### **C) API 接入服务（企业版）**

帮助企业：

- 内部自动化
- 智能客服
- 智能问答知识库

变现方式：

- 项目制报价
- 月度维护费

--------

###  技术实现（可落地）

### 📌 **后端**

- Node.js / Python Flask / FastAPI
- 接入 OpenAI SDK / Anthropic SDK / Google Gemini API

### 📌 **前端**

- React / Vue
- 输入框 + 模型选择 + 付费额度展示

### 📌 **存储与数据**

- Redis 缓存
- MySQL 存储用户消耗与付费记录

### 📌 **计费与权限管理**

- 按 token 计费
- 会员权限解锁
- 免费额度控制

### 📌 **部署**

- 放在现有阿里云轻量服务器（1台 阿里云轻量服务器ESC，5M带宽，40G存储，一个固定IP，一个已经备案的域名 qizhi520.cn）
- 反向代理 NGINX
- HTTPS + CDN 优化

### 收益模型（实战可行）

| 模式     | 收费方式      | 预期        | 关键动作         |
| -------- | ------------- | ----------- | ---------------- |
| 会员制   | 10–99 元/月   | 稳定现金流  | 提供更大配额     |
| 按量付费 | 0.1–0.5 元/条 | 量大收益高  | 轻量用户付费入口 |
| 企业 API | 项目制        | 5K–30K/项目 | 做自动化解决方案 |

------

### 营销与获客方案（有落地执行路径）

📍 **1) 社交内容引流**
 抖音/小红书：

- AI 生成案例展示
- 模型对比实测
- 提纲写作视频

📍 **2) 微信私域 + 社群**
 介绍会员权益、免费工具入口、用户体验反馈

📍 **3) 公众号/SEO**

📍 **4) 合作渠道**
 与教育/内容写作/电商运营等机构合作推广

### ⚠️ 合法合规提示

✔️ 不要未经授权分发或镜像 OpenAI 或其他厂商内容
 ✔️ 使用合法 API 并付费
 ✔️ 遵守当地隐私与数据保护法律
 ✔️ 如果提供服务给他人，需清晰用户条款、免责协议

------

### 📌 立即可执行的 7 步行动方案

1. **注册并申请合法 OpenAI API Key**（付费）
2. **后端框架搭建**（FastAPI）
3. **前端 UI 上线域名**（React + NGINX）
4. **模型调用层**（封装 GPT/Anthropic API）
5. **计费与权限管理插件**（Stripe / 国内支付）
6. **上线 Beta 测试 + 社群获取首批用户**
7. **迭代付费功能 + 企业 API**

----------

## 一、模板库的产品定位（非常关键）

### 1. 模板库如何“产品化”（而不是 Prompt 堆砌）

> 你现在要牢牢记住一句话：
> **模板不是功能，是商品。**

#### 2. 模板库不是 Prompt 集合，而是「AI 应用」

你对外的说法永远是：

> ❌「我们有 50 个 Prompt」
> ✅「我们内置 50 个 AI 应用模板，填信息直接出结果」

**核心价值**：

- 用户不用学 Prompt
- 用户不用想结构
- 用户只做“填空题”

### 1️⃣ 模板在产品里的真实形态

❌ 错误做法

- 一个 textarea
- 一个“生成”按钮
- 一段 Prompt

✅ 正确做法（你应该做的）

- 每个模板 = 一个**小应用**
- 用户只看到：**填信息 → 出结果**
- Prompt 永远在后端，永远不暴露

#### 前端用户看到的结构

```
【模板名称】
用途说明（1句话）
--------------------------------
[ 输入：主题 ]
[ 输入：目标人群 ]
[ 选择：风格（下拉） ]
--------------------------------
【生成结果】
```

👉 用户感知的是：
 **“我在用一个工具”**，而不是“我在用 AI”。

------

### 2️⃣ 模板的三种价值层级（直接决定你卖多少钱）

#### 🟢 L1：生成型（免费/引流）

- 选题
- 标题
- 简单文案

👉 目的：让用户爽一次

------

#### 🟡 L2：结果型（29–69 会员核心）

- 脚本
- 方案
- 话术
- 汇报

👉 目的：**帮用户省 1–3 小时**

------

#### 🔴 L3：决策型（高客单/企业）

- 定价策略
- 转化流程
- 运营方案
- SOP

👉 目的：**帮用户赚钱 / 少犯错**

你现在做的 **50 个模板**，是 **L2 + 少量 L3**，这是最赚钱的组合。

------

## 二、模板如何直接帮你赚钱（非常重要）

### 1️⃣ 模板 = 付费理由

- 免费用户：只能用 5 个
- 付费用户：解锁“解决具体问题”的模板

👉 用户买的不是 AI，是：
 **“这个模板正好解决我现在的问题”**

------

### 2️⃣ 模板 = 护城河

- Prompt 不公开
- 结构不断优化
- 用户习惯后不会换平台

------

### 3️⃣ 模板 = 运营内容

你每天的内容就是：

> 用【模板 X】
> 1 分钟生成【结果 Y】
> 对比不用 AI 的效率

-----

### 4️⃣ 你现在可以立刻执行的 7 步

- 先实现 **10 个模板**（短视频 + 销售 + 工作）
-  上线 **模板专属入口**（首页第一屏）
-  免费用户只能用 **3 次模板**
-  私域引流：

> 评论“模板”，送 3 个
> ⑤ 社群每天演示 1 个模板
> ⑥ 29 元会员解锁全部
> ⑦ 持续新增模板（每周 2–3 个）

------

## 三、模板的「标准输出规范」（工程 + 商业都很重要）

你一定要强制 AI **结构化输出**，否则用户会觉得“乱、像 AI”。

### 1. 标准输出格式（强烈建议你统一）

```text
【一、核心结论】
（直接给用户能用的结果）

【二、具体内容】
1.
2.
3.

【三、使用建议】
- 场景
- 注意事项
```

#### 示例：短视频脚本模板（输出规范）

```text
【视频定位】
一句话说清主题与人群

【口播脚本】
开头（3秒钩子）：
中段（给价值）：
结尾（行动号召）：

【拍摄建议】
- 场景
- 语速
- 镜头
```

👉 这样用户会觉得：
 **“这不是聊天，这是成品”**



### 2. 模板与「套餐定价」的强绑定（你就靠这个赚钱）

#### 1️⃣ 模板 = 会员卖点（非常重要）

你要在页面上**明确写出来**：

**免费用户**

- 5 个基础模板
- 每个模板每天 1 次
- 不能用“高质量模型”

**基础会员（29 元）**

- 30 个模板
- 不限次数（受额度限制）
- 可导出

**高级会员（69 元）**

- 50 个模板
- 高质量模型
- 优先生成
- 新模板抢先体验

👉 **用户不是为“额度”付费，是为“模板解锁”付费。**

------

#### 2️⃣ 模板升级 = 你后期涨价的理由

你以后可以这样涨价，而用户几乎不反感：

- “新增 10 个电商模板”
- “新增 5 个企业 SOP 模板”
- “新增 AI 去味增强模板”



### 3. 模板库如何反向推动你的获客（增长核心）

#### 1️⃣ 内容即模板演示（最省力）

你每天发内容，只做一件事：

> 用【模板 X】
> 30 秒生成【结果 Y】
> 对比不用 AI 的效率

#### 示例短视频脚本

```
我以前写一条招生文案要2小时
现在用我这个AI模板
30秒直接出3版
你只需要填3个信息
```

------

#### 2️⃣ 私域引流固定钩子（照抄）

> 评论区打「模板」
> 我送你 3 个我正在用的 AI 模板
> 直接能用，不用学 Prompt

------

#### 3️⃣ 社群运营只围绕模板（不讲技术）

每天一个固定动作：

- 今天用哪个模板
- 解决什么问题
- 给出一个真实结果

👉 **用户会形成依赖**。



### 4. 模板库的“护城河设计”（防抄、防卷）

很多人会抄 Prompt，但**抄不走你这三点**：

#### 1️⃣ Prompt 分层

- system role（角色）
- structure（结构）
- rules（规则）
- style（风格）

👉 单条 Prompt 超 300–600 字，且持续迭代。

------

#### 2️⃣ 模板组合

- 一个结果 = 多轮内部 Prompt
- 用户永远只看到最终输出

------

#### 3️⃣ 用户习惯

- 填空式
- 固定结构
- 导出即用

👉 **习惯比技术更难被复制。**



### 5. 现在最该做的 14 天执行清单（不需要再思考）

#### 第 1–3 天

- 从 50 个里选 **10 个模板**
- 优先：短视频 / 销售 / 工作

#### 第 4–7 天

- 上线模板页面
- 免费用户限制 3 次
- 会员解锁

#### 第 8–14 天

- 每天发 2 条模板演示内容
- 引导私域
- 社群 7 天促转化

🎯 **目标**：

> 100 注册 → 20 付费 → 跑通第一波现金流

--------

## 四、所有 Prompt 必须统一遵守的「商用级 Prompt 架构」

平台里的 **每一个模板 Prompt 都按这个 5 层结构来**，这是**商用品质与野 Prompt 的本质区别**。

### ✅ Prompt 五层结构（强烈建议你固化）

> ⚠️ 非常重要：
> **你平台里所有模板 Prompt 必须统一这一结构**，这是你产品专业度的根基。

```text
【SYSTEM 角色层】角色与专业边界
【TASK 任务层】明确交付目标
【INPUT 变量层】用户变量
【RULE 规则层】强约束规则
【OUTPUT 输出规范层】结构化输出规范
```

你**永远不要**只写一句 Prompt。

------

#### ① SYSTEM（角色层）

作用：**让模型进入稳定专业状态**

**示例模板**

```text
你是一名拥有10年以上经验的【短视频编导/销售总监/资深运营/教研专家】，
熟悉【平台/行业/人群】，
擅长将复杂问题拆解为可直接执行的方案。
```

------

#### ② TASK（任务层）

作用：**告诉模型你要“干什么结果”，不是“聊什么”**

```text
你的任务是：根据用户提供的信息，生成一份【可直接使用的完整成果】，而不是解释过程。
```

------

#### ③ INPUT（变量层）

作用：**把用户输入精准嵌进去**

```text
用户输入信息如下：
- 主题：{{topic}}
- 目标人群：{{audience}}
- 风格偏好：{{style}}
```

------

#### ④ RULE（规则层）【商用最关键】

作用：**防跑偏、防废话、防 AI 味**

```text
必须遵守以下规则：
1. 不要解释你在做什么
2. 不要出现“作为AI”“建议你”等表述
3. 语言自然、口语化、像真人
4. 输出必须具体，可直接使用
5. 不允许输出多余总结
```

------

#### ⑤ OUTPUT（输出规范层）

作用：**让结果像“产品交付件”**

```text
请严格按以下结构输出：

【一、核心结果】
……

【二、具体内容】
1.
2.
3.

【三、使用说明】
……
```

-------------

## 五、模板库技术结构设计（工程可直接用）

### 1️⃣ 模板数据结构（数据库 / JSON）

```json
{
  "template_id": "short_video_script_v1",
  "category": "内容创作",
  "title": "短视频口播脚本",
  "desc": "30-60秒短视频，适合抖音/视频号",
  "inputs": [
    { "key": "topic", "label": "视频主题", "type": "text", "required": true },
    { "key": "audience", "label": "目标人群", "type": "text", "required": true },
    { "key": "style", "label": "风格", "type": "select", "options": ["专业", "接地气", "故事化"] }
  ],
  "prompt": "你是一名短视频爆款编剧，请为{{audience}}，围绕{{topic}}，用{{style}}风格，生成一条30-60秒的口播脚本，要求：开头3秒强钩子，中段给价值，结尾行动号召。",
  "output_format": "脚本结构化输出",
  "credit_cost": 2,
  "vip_only": false
}
```

👉 **重点**：

- Prompt 永远在后端
- 前端只渲染 `inputs`
- 模板 = 一次“完整 AI 工作流”

------

### 2️⃣ 模板分级策略（直接用于收费）

| 等级     | 权限                  |
| -------- | --------------------- |
| 免费     | 5–8 个基础模板        |
| 基础会员 | 30 个常用模板         |
| 高级会员 | 全部模板 + 高质量模型 |

------

## 六、50 个高变现 AI 模板Prompt 设计方法（完整清单）

#### 统一商用 Prompt 基座（建议在代码里做“公共前缀 + 模板差异段”）

**公共 RULE（所有模板通用）**——建议后端拼接时固定加上：

```text
【通用规则】
- 直接给可用结果，不解释推理过程
- 禁止出现“作为AI/我建议/希望对你有帮助”等AI腔
- 输出具体、可执行、可复制粘贴
- 语言自然、符合中文习惯
- 不输出与任务无关的内容
```

> 以下每个模板只写“模板差异段”，可以直接整段存库（包含YSTEM/TASK/INPUT/RULE/OUTPUT）。



------

# 第二部分：模板后台设计（工程级，可直接开发）

> 目标：
>
> - **运营人员不用改代码也能加模板**
> - 不改代码也能加模板，Prompt 永不泄露，模板本身可变现
> - 模板可以分级、下线、调整价格
> - Prompt 永远在后台

------

#### 后台要解决的核心问题

- 运营能**新增/改模板**，不用改代码
- 模板可**分级、定价、上下线、AB测试**
- Prompt 永不泄露（管理端可见，用户端不可见）
- 能看数据：**哪个模板最赚钱、哪个模板带来付费**

## 一、模板后台功能结构

```
模板后台
├── 模板管理
│   ├── 新建 / 编辑
│   ├── 上线 / 下线
│   ├── 权限控制
│
├── 表单配置
│   ├── 输入字段配置
│   ├── 下拉选项
│
├── Prompt 管理
│   ├── 加密存储
│   ├── 版本控制
│
├── 使用统计
│   ├── 调用次数
│   ├── 转化来源
│
└── 权限与套餐
```

------

## 二、模板后台核心模块

### 1️⃣ 模板管理（最重要）

**字段设计**（模板主表 `ai_templates`）

| 字段          | 说明                          |
| ------------- | ----------------------------- |
| template_id   | 唯一ID                        |
| template_key  | UNIQUE                        |
| title         | 模板名称                      |
| category      | 分类                          |
| description   | 描述                          |
| prompt        | 完整 Prompt（加密存储）       |
| input_schema  | JSON 表单                     |
| output_schema | JSON                          |
| credit_cost   | 单次消耗                      |
| daily_limit   | 可选：每日次数限制            |
| vip_level     | 权限等级（0免费 1基础 2高级） |
| status        | 上/下线                       |
| sort_order    | 订单数                        |
| created_at    | 创建时间                      |
| updated_at    | 更新时间                      |

### 2️⃣ 表单可视化配置（关键）

运营在后台配置：

#### 示例 `input_schema`

```json
[
  { "key": "topic", "label": "主题", "type": "text", "required": true },
  { "key": "audience", "label": "目标人群", "type": "text" },
  { "key": "style", "label": "风格", "type": "select", "options": ["专业","口语","故事化"] }
]
```

👉 前端自动渲染，无需写死页面。

### 3️⃣ 权限与套餐绑定

```text
免费用户：vip_level = 0
基础会员：vip_level = 1
高级会员：vip_level = 2
```

模板可配置：

- 仅会员
- 仅高级会员
- 限每日次数

### 4️⃣ 模板统计（你赚钱靠这个）

- 使用次数
- 付费转化来源模板
- 热度排行
- 废弃模板（低使用）

------

## 三、模板执行流程（后端逻辑）

```
用户点击模板
→ 校验会员等级
→ 校验额度
→ 填表单
→ 后端拼 Prompt
→ 调用模型
→ 扣额度
→ 返回结构化结果
→ 记录日志
```

### 表单可视化配置（核心）

#### 示例 `input_schema`

```json
[
  { "key": "topic", "label": "主题", "type": "text", "required": true },
  { "key": "audience", "label": "目标人群", "type": "text" },
  { "key": "style", "label": "风格", "type": "select", "options": ["专业","口语","故事化"] }
]
```

👉 前端自动渲染，无需写死页面。

### 为什么这套设计非常“值钱”

1️⃣ Prompt 永远不泄露
2️⃣ 模板可持续升级
3️⃣ 可拉开与“Prompt 网站”的差距
4️⃣ 运营人员即可驱动增长
5️⃣ 你可以卖 **“模板本身”**
 ✔ 运营能独立工作
 ✔ 模板本身可售卖
 ✔ 极强复用性



> **你现在做的不是“AI工具”，而是“AI解决方案商店”。**

你已经具备：

- 产品 ✔
- 技术 ✔
- 商业模型 ✔

下一步，只剩下 **放大**。

#### 最关键的一句话（你一定要记住）

> **你不是在卖 AI，你是在卖“已经替用户想好的一整套解决方案”。**

## 四、数据表设计（SQL 级）

### 1）模板主表 `ai_templates`

```
CREATE TABLE ai_templates (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  template_key VARCHAR(64) NOT NULL UNIQUE,
  title VARCHAR(120) NOT NULL,
  category VARCHAR(50) NOT NULL,
  description VARCHAR(255) DEFAULT '',
  prompt TEXT NOT NULL,
  input_schema JSON NOT NULL,
  output_schema JSON NULL,
  credit_cost INT NOT NULL DEFAULT 1,
  vip_level INT NOT NULL DEFAULT 0,      -- 0免费 1基础 2高级
  daily_limit INT DEFAULT NULL,          -- 可选：每日次数限制
  status TINYINT NOT NULL DEFAULT 1,     -- 1上线 0下线
  sort_order INT NOT NULL DEFAULT 0,
  created_at DATETIME NOT NULL,
  updated_at DATETIME NOT NULL
);
```

### 2）模板版本表 `ai_template_versions`（强烈建议做）

```sql
CREATE TABLE ai_template_versions (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  template_id BIGINT NOT NULL,
  version INT NOT NULL,
  prompt TEXT NOT NULL,
  input_schema JSON NOT NULL,
  credit_cost INT NOT NULL,
  vip_level INT NOT NULL,
  status TINYINT NOT NULL DEFAULT 1,
  changelog VARCHAR(255) DEFAULT '',
  created_by BIGINT NULL,
  created_at DATETIME NOT NULL,
  UNIQUE(template_id, version)
);
```

### 3）模板调用日志 `ai_template_runs`

```sql
CREATE TABLE ai_template_runs (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  user_id BIGINT NOT NULL,
  template_key VARCHAR(64) NOT NULL,
  model_id VARCHAR(64) NOT NULL,
  input_json JSON NOT NULL,
  output_text MEDIUMTEXT NOT NULL,
  credits_cost INT NOT NULL,
  success TINYINT NOT NULL DEFAULT 1,
  error_msg VARCHAR(255) DEFAULT '',
  created_at DATETIME NOT NULL,
  INDEX(user_id),
  INDEX(template_key),
  INDEX(created_at)
);
```

### 4）模板转化归因（可选但很赚钱）`ai_conversion_events`

```sql
CREATE TABLE ai_conversion_events (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  user_id BIGINT NOT NULL,
  event_type VARCHAR(32) NOT NULL,   -- view_template, run_template, pay, upgrade
  template_key VARCHAR(64) NULL,
  meta JSON NULL,
  created_at DATETIME NOT NULL,
  INDEX(user_id),
  INDEX(template_key)
);
```

----------

# 第三部分：后台页面和功能设计（运营可用）

## 一、后台主要页面

### 页面 1：模板列表

- 搜索：title / template_key / category
- 筛选：vip_level / status
- 字段：标题、分类、等级、单次消耗、状态、近7天使用次数、转化率（可选）
- 操作：编辑、复制、上下线、查看数据、版本历史

### 页面 2：模板编辑（核心）

- 基本信息：标题/分类/描述/排序/状态
- 表单配置：可视化编辑 `input_schema`（增删字段、类型、下拉选项、默认值、必填）
- Prompt 编辑器：大文本框 + 变量检测（检测 {{xxx}} 是否都在 input_schema 中）
- 输出规范（可选）：`output_schema`（让前端能一键复制“分段块”）
- 权限与定价：vip_level、credit_cost、daily_limit
- 保存时：自动写入版本表

### 页面 3：版本管理

- 列出版本号、changelog、创建人、创建时间
- 支持：回滚到某版本、对比差异（diff）

### 页面 4：数据看板

- 模板热度Top20（使用次数/人均使用/留存）
- 模板带来的付费转化Top（run→pay）
- 失败率/错误率Top（排查上游模型或Prompt问题）
- 成本监控：按模型维度的调用量与消耗（后期做精确token）

## 二、API 设计（后端直接照着写）

### 1. 主要API功能

#### 1）用户端模板 API

- `GET /api/templates`
    - 返回：按权限过滤后的模板列表（含input_schema、credit_cost、vip_level）
- `GET /api/templates/{template_key}`
    - 返回模板详情（不返回prompt）
- `POST /api/templates/{template_key}/run`
    - body：`{ model_id, inputs: {...} }`
    - 流程：鉴权→校验vip→校验额度→拼prompt→调用模型→扣费→落日志→返回结构化结果

#### 2）管理端 API（加 admin 权限）

- `POST /api/admin/templates` 新建
- `PUT /api/admin/templates/{id}` 编辑
- `POST /api/admin/templates/{id}/publish` 上线
- `POST /api/admin/templates/{id}/unpublish` 下线
- `GET /api/admin/templates/{id}/versions` 版本列表
- `POST /api/admin/templates/{id}/rollback?version=n` 回滚
- `GET /api/admin/metrics/templates` 看板数据

## 2. 基于FastAPI MVP上扩展出模板系统后端骨架

可跑、可落库、可扩展，包含：**模板表 / 版本表 / 调用日志 / 用户端模板列表&运行 / 管理端CRUD&上下线&版本回滚 / 变量校验 / 基础Admin鉴权**。

> 说明：仍用你现在的 **SQLite + SQLAlchemy**，后续换 MySQL 基本只改 DB_URL。
> 下面是“直接复制粘贴覆盖/新增文件”的形式。

## 1) 新增/修改文件一览

```
backend/
  db.py                # 不变
  settings.py          # 增加 ADMIN_KEY
  models.py            # 增加 Template/TemplateVersion/TemplateRun/ConversionEvent
  templates.py         # 新增：模板渲染/变量校验/公共函数
  main.py              # 增加模板API与管理端API
```

### 2) settings.py（新增 ADMIN_KEY）

```python
# backend/settings.py
import os

JWT_SECRET = os.getenv("JWT_SECRET", "CHANGE_ME_TO_A_LONG_RANDOM_SECRET")
JWT_ALG = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", "10080"))

OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "")
OPENAI_BASE_URL = os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1")

DEFAULT_FREE_CREDITS = int(os.getenv("DEFAULT_FREE_CREDITS", "30"))

# NEW: admin key for /api/admin/*
ADMIN_KEY = os.getenv("ADMIN_KEY", "CHANGE_ME_ADMIN_KEY")
```

### 3) models.py（新增表）

```python
# backend/models.py
from sqlalchemy import String, Integer, DateTime, Text, ForeignKey, Boolean
from sqlalchemy.orm import Mapped, mapped_column, relationship
from datetime import datetime
from db import Base

class User(Base):
    __tablename__ = "users"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    email: Mapped[str] = mapped_column(String(255), unique=True, index=True)
    password_hash: Mapped[str] = mapped_column(String(255))
    credits: Mapped[int] = mapped_column(Integer, default=0)
    vip_level: Mapped[int] = mapped_column(Integer, default=0)  # NEW: 0 free, 1 basic, 2 pro
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    chats = relationship("ChatLog", back_populates="user")
    template_runs = relationship("TemplateRun", back_populates="user")


class ChatLog(Base):
    __tablename__ = "chat_logs"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    model: Mapped[str] = mapped_column(String(64))
    prompt: Mapped[str] = mapped_column(Text)
    response: Mapped[str] = mapped_column(Text)
    cost_credits: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    user = relationship("User", back_populates="chats")


# ---------------------------
# Templates system
# ---------------------------

class Template(Base):
    __tablename__ = "ai_templates"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    template_key: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    title: Mapped[str] = mapped_column(String(120))
    category: Mapped[str] = mapped_column(String(50), index=True)
    description: Mapped[str] = mapped_column(String(255), default="")
    prompt: Mapped[str] = mapped_column(Text)  # keep on server; never send to user
    input_schema: Mapped[str] = mapped_column(Text)  # JSON string for sqlite
    output_schema: Mapped[str] = mapped_column(Text, default="")  # optional JSON string
    credit_cost: Mapped[int] = mapped_column(Integer, default=1)
    vip_level: Mapped[int] = mapped_column(Integer, default=0)  # 0 free, 1 basic, 2 pro
    daily_limit: Mapped[int | None] = mapped_column(Integer, nullable=True)
    status: Mapped[int] = mapped_column(Integer, default=1)  # 1 online, 0 offline
    sort_order: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    versions = relationship("TemplateVersion", back_populates="template")


class TemplateVersion(Base):
    __tablename__ = "ai_template_versions"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    template_id: Mapped[int] = mapped_column(ForeignKey("ai_templates.id"), index=True)
    version: Mapped[int] = mapped_column(Integer)
    prompt: Mapped[str] = mapped_column(Text)
    input_schema: Mapped[str] = mapped_column(Text)
    credit_cost: Mapped[int] = mapped_column(Integer, default=1)
    vip_level: Mapped[int] = mapped_column(Integer, default=0)
    status: Mapped[int] = mapped_column(Integer, default=1)
    changelog: Mapped[str] = mapped_column(String(255), default="")
    created_by: Mapped[int | None] = mapped_column(Integer, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    template = relationship("Template", back_populates="versions")


class TemplateRun(Base):
    __tablename__ = "ai_template_runs"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    template_key: Mapped[str] = mapped_column(String(64), index=True)
    model_id: Mapped[str] = mapped_column(String(64))
    input_json: Mapped[str] = mapped_column(Text)  # JSON string
    output_text: Mapped[str] = mapped_column(Text)
    credits_cost: Mapped[int] = mapped_column(Integer, default=1)
    success: Mapped[bool] = mapped_column(Boolean, default=True)
    error_msg: Mapped[str] = mapped_column(String(255), default="")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    user = relationship("User", back_populates="template_runs")


class ConversionEvent(Base):
    __tablename__ = "ai_conversion_events"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(Integer, index=True)
    event_type: Mapped[str] = mapped_column(String(32), index=True)  # view_template/run_template/pay/upgrade
    template_key: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    meta: Mapped[str] = mapped_column(Text, default="")  # JSON string
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
```

### 4) templates.py（新增：变量校验 + prompt 渲染）

```python
# backend/templates.py
import json
import re
from typing import Any, Dict, List, Tuple

VAR_PATTERN = re.compile(r"\{\{\s*([a-zA-Z0-9_]+)\s*\}\}")

def parse_json(text: str, default: Any):
    try:
        return json.loads(text) if text else default
    except Exception:
        return default

def extract_vars_from_prompt(prompt: str) -> List[str]:
    return sorted(set(VAR_PATTERN.findall(prompt or "")))

def validate_inputs_against_schema(input_schema: List[Dict[str, Any]], inputs: Dict[str, Any]) -> Tuple[bool, str]:
    """
    input_schema item example:
    { "key":"topic", "label":"主题", "type":"text", "required":true, "options":[...] }
    """
    schema_keys = {f.get("key") for f in input_schema if isinstance(f, dict)}
    # required check
    for f in input_schema:
        if not isinstance(f, dict):
            continue
        key = f.get("key")
        required = bool(f.get("required"))
        if required and (key not in inputs or inputs.get(key) in (None, "", [])):
            return False, f"Missing required field: {key}"

        # basic select option validation
        if f.get("type") == "select" and key in inputs and "options" in f:
            if inputs[key] not in f["options"]:
                return False, f"Invalid option for {key}"

    # allow extra fields? For safety we can reject unknown fields:
    for k in inputs.keys():
        if k not in schema_keys:
            return False, f"Unknown input field: {k}"

    return True, ""

def validate_prompt_vars(prompt: str, input_schema: List[Dict[str, Any]]) -> Tuple[bool, str]:
    vars_in_prompt = set(extract_vars_from_prompt(prompt))
    schema_keys = set([f.get("key") for f in input_schema if isinstance(f, dict) and f.get("key")])

    missing = vars_in_prompt - schema_keys
    if missing:
        return False, f"Prompt contains vars not in schema: {sorted(missing)}"
    # It's ok if schema has keys not used in prompt.
    return True, ""

def render_prompt(prompt: str, inputs: Dict[str, Any]) -> str:
    # simple safe replacement
    def repl(m):
        key = m.group(1)
        v = inputs.get(key, "")
        if isinstance(v, (dict, list)):
            return json.dumps(v, ensure_ascii=False)
        return str(v)
    return VAR_PATTERN.sub(repl, prompt)
```

### 5) main.py（扩展：模板API + 管理端API）

把下面代码块按注释插入到你现有 `backend/main.py` 中（最简单：直接用这份替换 main.py 然后把你原 chat 接口保留/合并）。

```python
# backend/main.py
from fastapi import FastAPI, Depends, HTTPException, Header
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel, EmailStr, Field
from sqlalchemy.orm import Session
from datetime import datetime, timedelta
from jose import jwt, JWTError
from passlib.context import CryptContext
import requests
import math
import json

from db import SessionLocal, engine, Base
from models import User, ChatLog, Template, TemplateVersion, TemplateRun, ConversionEvent
from settings import (
    JWT_SECRET, JWT_ALG, ACCESS_TOKEN_EXPIRE_MINUTES,
    OPENAI_API_KEY, OPENAI_BASE_URL,
    DEFAULT_FREE_CREDITS, ADMIN_KEY
)
from templates import parse_json, validate_inputs_against_schema, validate_prompt_vars, render_prompt

Base.metadata.create_all(bind=engine)

app = FastAPI(title="AI SaaS MVP + Templates")

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # production: replace with your domain
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

pwd = CryptContext(schemes=["bcrypt"], deprecated="auto")

def get_db():
    db = SessionLocal()
    try:
        yield db
    finally:
        db.close()

def create_access_token(sub: str):
    exp = datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    return jwt.encode({"sub": sub, "exp": exp}, JWT_SECRET, algorithm=JWT_ALG)

def get_current_user(db: Session, authorization: str = Header(default="")) -> User:
    if not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Missing bearer token")
    token = authorization.split(" ", 1)[1]
    try:
        payload = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALG])
        sub = payload.get("sub")
        if not sub:
            raise HTTPException(status_code=401, detail="Invalid token")
    except JWTError:
        raise HTTPException(status_code=401, detail="Invalid token")

    user = db.query(User).filter(User.email == sub).first()
    if not user:
        raise HTTPException(status_code=401, detail="User not found")
    return user

def require_admin(x_admin_key: str = Header(default="")):
    if x_admin_key != ADMIN_KEY:
        raise HTTPException(status_code=403, detail="Forbidden")
    return True

# --------------- Auth schemas ---------------
class AuthReq(BaseModel):
    email: EmailStr
    password: str

class AuthResp(BaseModel):
    token: str
    credits: int
    vip_level: int

# --------------- Chat schemas (keep your old) ---------------
class ChatReq(BaseModel):
    model: str = "gpt-4o-mini"
    message: str

class ChatResp(BaseModel):
    model: str
    reply: str
    cost_credits: int
    credits_left: int

# --------------- Template schemas ---------------
class TemplateListItem(BaseModel):
    template_key: str
    title: str
    category: str
    description: str
    input_schema: list
    output_schema: dict | None = None
    credit_cost: int
    vip_level: int
    status: int
    sort_order: int

class TemplateDetailResp(BaseModel):
    template_key: str
    title: str
    category: str
    description: str
    input_schema: list
    output_schema: dict | None = None
    credit_cost: int
    vip_level: int
    status: int

class TemplateRunReq(BaseModel):
    model_id: str = "gpt-4o-mini"
    inputs: dict = Field(default_factory=dict)

class TemplateRunResp(BaseModel):
    template_key: str
    model_id: str
    output_text: str
    credits_cost: int
    credits_left: int

# Admin
class AdminTemplateUpsertReq(BaseModel):
    template_key: str
    title: str
    category: str
    description: str = ""
    prompt: str
    input_schema: list
    output_schema: dict | None = None
    credit_cost: int = 1
    vip_level: int = 0
    daily_limit: int | None = None
    status: int = 1
    sort_order: int = 0
    changelog: str = ""

class AdminPublishReq(BaseModel):
    status: int  # 1 online, 0 offline

class AdminRollbackReq(BaseModel):
    version: int

# --- credits estimator for chat ---
def estimate_credits(text_in: str, text_out: str) -> int:
    total_chars = len(text_in) + len(text_out)
    return max(1, math.ceil(total_chars / 1000))

# --- Provider adapter (OpenAI) ---
def call_openai_chat(model: str, message: str) -> str:
    if not OPENAI_API_KEY:
        raise HTTPException(status_code=500, detail="OPENAI_API_KEY not set")
    url = f"{OPENAI_BASE_URL}/chat/completions"
    headers = {"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"}
    payload = {
        "model": model,
        "messages": [{"role": "user", "content": message}],
        "temperature": 0.7
    }
    r = requests.post(url, headers=headers, json=payload, timeout=60)
    if r.status_code != 200:
        raise HTTPException(status_code=502, detail=f"Upstream error: {r.text[:300]}")
    data = r.json()
    return data["choices"][0]["message"]["content"]

SUPPORTED_MODELS = [
    {"id": "gpt-4o-mini", "name": "GPT-4o mini（快/便宜）", "provider": "openai"},
    {"id": "gpt-4.1-mini", "name": "GPT-4.1 mini（更强）", "provider": "openai"},
]

# ---------------------------
# Auth endpoints
# ---------------------------
@app.post("/api/register", response_model=AuthResp)
def register(req: AuthReq, db: Session = Depends(get_db)):
    if db.query(User).filter(User.email == req.email).first():
        raise HTTPException(status_code=400, detail="Email already registered")
    u = User(
        email=req.email,
        password_hash=pwd.hash(req.password),
        credits=DEFAULT_FREE_CREDITS,
        vip_level=0
    )
    db.add(u); db.commit()
    token = create_access_token(u.email)
    return AuthResp(token=token, credits=u.credits, vip_level=u.vip_level)

@app.post("/api/login", response_model=AuthResp)
def login(req: AuthReq, db: Session = Depends(get_db)):
    u = db.query(User).filter(User.email == req.email).first()
    if not u or not pwd.verify(req.password, u.password_hash):
        raise HTTPException(status_code=401, detail="Invalid credentials")
    token = create_access_token(u.email)
    return AuthResp(token=token, credits=u.credits, vip_level=u.vip_level)

@app.get("/api/me")
def me(db: Session = Depends(get_db), authorization: str = Header(default="")):
    u = get_current_user(db, authorization)
    return {"email": u.email, "credits": u.credits, "vip_level": u.vip_level}

@app.get("/api/models")
def list_models():
    return {"models": SUPPORTED_MODELS}

# ---------------------------
# Optional: keep your /api/chat endpoint
# ---------------------------
@app.post("/api/chat", response_model=ChatResp)
def chat(req: ChatReq, db: Session = Depends(get_db), authorization: str = Header(default="")):
    u = get_current_user(db, authorization)
    if u.credits <= 0:
        raise HTTPException(status_code=402, detail="No credits left. Please upgrade.")

    model_info = next((m for m in SUPPORTED_MODELS if m["id"] == req.model), None)
    if not model_info:
        raise HTTPException(status_code=400, detail="Unsupported model")

    if model_info["provider"] == "openai":
        reply = call_openai_chat(req.model, req.message)
    else:
        raise HTTPException(status_code=400, detail="Provider not implemented")

    cost = estimate_credits(req.message, reply)
    if u.credits - cost < 0:
        raise HTTPException(status_code=402, detail="Not enough credits for this request")

    u.credits -= cost
    db.add(ChatLog(user_id=u.id, model=req.model, prompt=req.message, response=reply, cost_credits=cost))
    db.commit()

    return ChatResp(model=req.model, reply=reply, cost_credits=cost, credits_left=u.credits)

# ==========================================================
# Templates - USER APIs
# ==========================================================

def user_can_access_template(user: User, tpl: Template) -> bool:
    return (user.vip_level >= tpl.vip_level) and (tpl.status == 1)

@app.get("/api/templates", response_model=list[TemplateListItem])
def list_templates(db: Session = Depends(get_db), authorization: str = Header(default="")):
    u = get_current_user(db, authorization)
    q = db.query(Template).filter(Template.status == 1).order_by(Template.sort_order.desc(), Template.id.desc())
    out = []
    for t in q.all():
        if u.vip_level < t.vip_level:
            continue
        input_schema = parse_json(t.input_schema, [])
        output_schema = parse_json(t.output_schema, None)
        out.append(TemplateListItem(
            template_key=t.template_key,
            title=t.title,
            category=t.category,
            description=t.description,
            input_schema=input_schema,
            output_schema=output_schema,
            credit_cost=t.credit_cost,
            vip_level=t.vip_level,
            status=t.status,
            sort_order=t.sort_order
        ))
    return out

@app.get("/api/templates/{template_key}", response_model=TemplateDetailResp)
def get_template(template_key: str, db: Session = Depends(get_db), authorization: str = Header(default="")):
    u = get_current_user(db, authorization)
    t = db.query(Template).filter(Template.template_key == template_key).first()
    if not t or t.status != 1:
        raise HTTPException(status_code=404, detail="Template not found")
    if u.vip_level < t.vip_level:
        raise HTTPException(status_code=403, detail="Template requires higher plan")
    return TemplateDetailResp(
        template_key=t.template_key,
        title=t.title,
        category=t.category,
        description=t.description,
        input_schema=parse_json(t.input_schema, []),
        output_schema=parse_json(t.output_schema, None),
        credit_cost=t.credit_cost,
        vip_level=t.vip_level,
        status=t.status
    )

@app.post("/api/templates/{template_key}/run", response_model=TemplateRunResp)
def run_template(template_key: str, req: TemplateRunReq, db: Session = Depends(get_db), authorization: str = Header(default="")):
    u = get_current_user(db, authorization)
    t = db.query(Template).filter(Template.template_key == template_key).first()
    if not t or t.status != 1:
        raise HTTPException(status_code=404, detail="Template not found")
    if u.vip_level < t.vip_level:
        raise HTTPException(status_code=403, detail="Template requires higher plan")
    if u.credits < t.credit_cost:
        raise HTTPException(status_code=402, detail="Not enough credits. Please upgrade/top up.")

    # Validate model
    model_info = next((m for m in SUPPORTED_MODELS if m["id"] == req.model_id), None)
    if not model_info:
        raise HTTPException(status_code=400, detail="Unsupported model")
    if model_info["provider"] != "openai":
        raise HTTPException(status_code=400, detail="Provider not implemented")

    input_schema = parse_json(t.input_schema, [])
    ok, err = validate_inputs_against_schema(input_schema, req.inputs)
    if not ok:
        raise HTTPException(status_code=400, detail=err)

    ok2, err2 = validate_prompt_vars(t.prompt, input_schema)
    if not ok2:
        # This is a config error: admin should fix schema/prompt
        raise HTTPException(status_code=500, detail=f"Template config error: {err2}")

    # Render prompt
    full_prompt = render_prompt(t.prompt, req.inputs)

    # Call model
    run_log = TemplateRun(
        user_id=u.id,
        template_key=t.template_key,
        model_id=req.model_id,
        input_json=json.dumps(req.inputs, ensure_ascii=False),
        output_text="",
        credits_cost=t.credit_cost,
        success=True,
        error_msg=""
    )
    try:
        output_text = call_openai_chat(req.model_id, full_prompt)
        run_log.output_text = output_text
    except Exception as e:
        run_log.success = False
        run_log.error_msg = str(e)[:250]
        db.add(run_log); db.commit()
        raise

    # Deduct credits & save logs
    u.credits -= t.credit_cost
    db.add(run_log)

    # conversion event (optional)
    db.add(ConversionEvent(
        user_id=u.id,
        event_type="run_template",
        template_key=t.template_key,
        meta=""
    ))
    db.commit()

    return TemplateRunResp(
        template_key=t.template_key,
        model_id=req.model_id,
        output_text=output_text,
        credits_cost=t.credit_cost,
        credits_left=u.credits
    )

# ==========================================================
# Templates - ADMIN APIs
# ==========================================================

@app.post("/api/admin/templates")
def admin_create_template(req: AdminTemplateUpsertReq, _=Depends(require_admin), db: Session = Depends(get_db)):
    # Validate schema JSON
    if not isinstance(req.input_schema, list) or len(req.input_schema) == 0:
        raise HTTPException(status_code=400, detail="input_schema must be a non-empty list")
    ok, err = validate_prompt_vars(req.prompt, req.input_schema)
    if not ok:
        raise HTTPException(status_code=400, detail=err)

    existing = db.query(Template).filter(Template.template_key == req.template_key).first()
    if existing:
        raise HTTPException(status_code=400, detail="template_key already exists")

    t = Template(
        template_key=req.template_key,
        title=req.title,
        category=req.category,
        description=req.description,
        prompt=req.prompt,
        input_schema=json.dumps(req.input_schema, ensure_ascii=False),
        output_schema=json.dumps(req.output_schema, ensure_ascii=False) if req.output_schema else "",
        credit_cost=req.credit_cost,
        vip_level=req.vip_level,
        daily_limit=req.daily_limit,
        status=req.status,
        sort_order=req.sort_order,
        created_at=datetime.utcnow(),
        updated_at=datetime.utcnow()
    )
    db.add(t); db.commit(); db.refresh(t)

    v = TemplateVersion(
        template_id=t.id,
        version=1,
        prompt=t.prompt,
        input_schema=t.input_schema,
        credit_cost=t.credit_cost,
        vip_level=t.vip_level,
        status=t.status,
        changelog=req.changelog or "init",
        created_by=None,
        created_at=datetime.utcnow()
    )
    db.add(v); db.commit()
    return {"ok": True, "template_id": t.id, "version": 1}

@app.put("/api/admin/templates/{template_key}")
def admin_update_template(template_key: str, req: AdminTemplateUpsertReq, _=Depends(require_admin), db: Session = Depends(get_db)):
    t = db.query(Template).filter(Template.template_key == template_key).first()
    if not t:
        raise HTTPException(status_code=404, detail="Template not found")

    if not isinstance(req.input_schema, list) or len(req.input_schema) == 0:
        raise HTTPException(status_code=400, detail="input_schema must be a non-empty list")
    ok, err = validate_prompt_vars(req.prompt, req.input_schema)
    if not ok:
        raise HTTPException(status_code=400, detail=err)

    # compute next version
    latest = db.query(TemplateVersion).filter(TemplateVersion.template_id == t.id).order_by(TemplateVersion.version.desc()).first()
    next_ver = 1 if not latest else latest.version + 1

    # update template
    t.title = req.title
    t.category = req.category
    t.description = req.description
    t.prompt = req.prompt
    t.input_schema = json.dumps(req.input_schema, ensure_ascii=False)
    t.output_schema = json.dumps(req.output_schema, ensure_ascii=False) if req.output_schema else ""
    t.credit_cost = req.credit_cost
    t.vip_level = req.vip_level
    t.daily_limit = req.daily_limit
    t.status = req.status
    t.sort_order = req.sort_order
    t.updated_at = datetime.utcnow()

    # version snapshot
    v = TemplateVersion(
        template_id=t.id,
        version=next_ver,
        prompt=t.prompt,
        input_schema=t.input_schema,
        credit_cost=t.credit_cost,
        vip_level=t.vip_level,
        status=t.status,
        changelog=req.changelog or "",
        created_by=None,
        created_at=datetime.utcnow()
    )
    db.add(v)
    db.commit()
    return {"ok": True, "template_id": t.id, "version": next_ver}

@app.post("/api/admin/templates/{template_key}/publish")
def admin_publish_template(template_key: str, req: AdminPublishReq, _=Depends(require_admin), db: Session = Depends(get_db)):
    t = db.query(Template).filter(Template.template_key == template_key).first()
    if not t:
        raise HTTPException(status_code=404, detail="Template not found")
    t.status = 1 if req.status == 1 else 0
    t.updated_at = datetime.utcnow()
    db.commit()
    return {"ok": True, "status": t.status}

@app.get("/api/admin/templates/{template_key}/versions")
def admin_list_versions(template_key: str, _=Depends(require_admin), db: Session = Depends(get_db)):
    t = db.query(Template).filter(Template.template_key == template_key).first()
    if not t:
        raise HTTPException(status_code=404, detail="Template not found")
    versions = db.query(TemplateVersion).filter(TemplateVersion.template_id == t.id).order_by(TemplateVersion.version.desc()).all()
    return {"template_key": template_key, "versions": [
        {"version": v.version, "changelog": v.changelog, "created_at": v.created_at.isoformat(), "status": v.status}
        for v in versions
    ]}

@app.post("/api/admin/templates/{template_key}/rollback")
def admin_rollback(template_key: str, req: AdminRollbackReq, _=Depends(require_admin), db: Session = Depends(get_db)):
    t = db.query(Template).filter(Template.template_key == template_key).first()
    if not t:
        raise HTTPException(status_code=404, detail="Template not found")
    v = db.query(TemplateVersion).filter(
        TemplateVersion.template_id == t.id,
        TemplateVersion.version == req.version
    ).first()
    if not v:
        raise HTTPException(status_code=404, detail="Version not found")

    # apply rollback
    t.prompt = v.prompt
    t.input_schema = v.input_schema
    t.credit_cost = v.credit_cost
    t.vip_level = v.vip_level
    t.status = v.status
    t.updated_at = datetime.utcnow()

    # new version snapshot noting rollback
    latest = db.query(TemplateVersion).filter(TemplateVersion.template_id == t.id).order_by(TemplateVersion.version.desc()).first()
    next_ver = 1 if not latest else latest.version + 1
    new_v = TemplateVersion(
        template_id=t.id,
        version=next_ver,
        prompt=t.prompt,
        input_schema=t.input_schema,
        credit_cost=t.credit_cost,
        vip_level=t.vip_level,
        status=t.status,
        changelog=f"rollback to v{req.version}",
        created_by=None,
        created_at=datetime.utcnow()
    )
    db.add(new_v)
    db.commit()
    return {"ok": True, "rolled_back_to": req.version, "new_version": next_ver}
```

### 6) 初始化一个模板（用管理端 API）

#### 例：创建「短视频口播脚本」模板

请求头带 `X-Admin-Key: 你的ADMIN_KEY`

```bash
curl -X POST http://127.0.0.1:8000/api/admin/templates \
  -H "Content-Type: application/json" \
  -H "X-Admin-Key: CHANGE_ME_ADMIN_KEY" \
  -d '{
    "template_key":"short_video_script_v1",
    "title":"短视频口播脚本",
    "category":"内容创作",
    "description":"30-60秒口播脚本，含钩子与行动号召",
    "prompt":"【SYSTEM】你是一名拥有10年以上经验的短视频爆款编导，精通抖音和视频号平台规则。\n【TASK】生成一条可直接拍摄、可直接朗读的短视频口播脚本。\n【INPUT】- 视频主题：{{topic}}\n- 目标人群：{{audience}}\n- 表达风格：{{style}}\n【RULE】1. 禁止解释 2. 口语化 3. 开头3秒强钩子 4. 不出现结构提示词 5. 结尾有行动指令\n【OUTPUT】【视频定位】一句话说明\n【完整口播脚本】（30-60秒）\n【拍摄建议】- 场景 - 语速 - 镜头",
    "input_schema":[
      {"key":"topic","label":"视频主题","type":"text","required":true},
      {"key":"audience","label":"目标人群","type":"text","required":true},
      {"key":"style","label":"风格","type":"select","required":true,"options":["专业","接地气","故事化"]}
    ],
    "credit_cost":2,
    "vip_level":0,
    "status":1,
    "sort_order":100,
    "changelog":"init"
  }'
```

### 7) 需要注意的两个“立刻可增强”的点（不影响骨架跑通）

1. **daily_limit 未实现**：现在骨架里存了字段但没做“按天次数限制”。要加也简单：在 `run_template` 里按当天日期统计 `TemplateRun` 次数即可。
2. **prompt 不建议存纯文本**：MVP 没问题；要更安全可加密存储或至少在管理端返回时不回传 prompt。

### 8) 需完善 FastAPI 模板系统骨架的以下功能

1. ✅ **daily_limit（每日次数限制）**：按“用户 + 模板 + 当天”限制调用次数
2. ✅ **模板使用统计 Top 榜**：管理端和运营端都能看（7天/30天）
3. ✅ **模板“公共前缀”拼接机制**：SYSTEM/RULE 统一管理，运营只维护“差异段”（模板 body），一处更新全站生效。这样以后运营只维护 `prompt_body`，你想统一风格/规则，只改 PromptBase。
4. ✅ **run → pay 转化显示/推理归因榜**：告诉你“哪个模板最促成付费”。在用户端打开“升级弹窗/充值页”时，先记录 `event_type=view_paywall` 并带 `template_key`，支付成功后再关联订单，这样归因更稳定。
5. ✅ **失败率/异常榜**：告诉你“哪个模板最容易报错/失败，优先修哪个”



- 新增一张表：`ai_prompt_bases`（公共前缀：system_base、rules_base 等）

    ```
    class PromptBase(Base):
        __tablename__ = "ai_prompt_bases"
        id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
        name: Mapped[str] = mapped_column(String(64), unique=True, index=True)  # e.g. "default"
        system_base: Mapped[str] = mapped_column(Text)  # shared SYSTEM
        rules_base: Mapped[str] = mapped_column(Text)   # shared RULES
        output_base: Mapped[str] = mapped_column(Text, default="")  # optional shared OUTPUT hints
        created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
        updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    ```

- 模板表新增一个字段：`prompt_body`（只存差异段）

    **在 Template 表增加 `prompt_body`**

    ```python
    # 在 Template 类里新增字段（保留 prompt 也行，但建议逐步弃用 prompt）
    prompt_body: Mapped[str] = mapped_column(Text, default="")  # NEW: only template-specific part
    ```

    > 建议策略：
    >
    > - 现在先保留 `prompt` 字段兼容旧模板
    > - 新模板只用 `prompt_body`
    > - 运行时若 `prompt_body` 为空，则 fallback 用 `prompt`

- 调用模板时：`final_prompt = system_base + rules_base + prompt_body`（并支持变量替换）

- `daily_limit` 生效：超过次数直接 429/400（我用 429 更合理）

- 新增统计 API：

    - `GET /api/admin/metrics/templates/top?days=7&metric=runs`（Top 使用）
    - `GET /api/admin/metrics/templates/top?days=7&metric=credits`（Top 消耗）
    - `GET /api/admin/metrics/templates/top?days=7&metric=users`（Top 使用人数）

--------

## 三、安全与防泄露（必须做）

- **用户端永远不下发 prompt**
- prompt 存库可做两层保护：
    1. 只允许 admin API 读取
    2. 数据库层面加密（可选）
- 管理端操作日志：谁改了哪个模板、何时改的、改了什么版本
- 模板 run 日志中敏感 input 可脱敏（如手机号）

## 四、工作任务

1. 先把**10个黄金模板**上站（短视频/销售/职场为主）
2. 做后台“模板新增/编辑/上下线/版本”四件套（先别贪看板）
3. 跑通：模板→用户用→扣费→日志
4. 再补齐其余40模板到后台（复制改字段+Prompt）
5. 最后做看板和转化归因（它决定你后续怎么增长）